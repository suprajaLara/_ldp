...
import org.example.aroundWithLogger.Account;
...
public class MyDemoLoggingAspectForPointcut {
    ...
    @Around("execution(* org.example.aroundWithLogger.service.*.getFortune(..))")
    ...
    @After("execution(* org.example.aroundWithLogger.dao.AccountDAO.findAccounts(..))")
    ...
    @AfterThrowing(
            pointcut = "execution(* org.example.aroundWithLogger.dao.AccountDAO.findAccounts(..))",
            throwing = "exception"
    )
    ...
    @Before("org.example.aroundWithLogger.aspect.AopExpressions.excludeGetterSetter()") //reference to pointcut expression (already declared above)
